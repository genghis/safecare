...
const STRINGS: Record<string, StringEntry> = {
  ...
  // --- WhatsApp setup wizard (Step 4) ---
  'dashboard.setup.whatsappSetup': { en: 'Set Up WhatsApp', es: 'Configurar WhatsApp', ar: 'إعداد واتساب', so: 'Dejinta WhatsApp', fr: 'Configurer WhatsApp', zh: '设置 WhatsApp', uk: 'Налаштувати WhatsApp' },
  'dashboard.setup.whatsappSetupDesc': { en: 'WhatsApp is the easiest way to send delivery notifications. Three simple steps to get it running.', es: 'WhatsApp es la forma más fácil de enviar notificaciones de entrega. Tres pasos sencillos.', ar: 'واتساب هو أسهل طريقة لإرسال إشعارات التوصيل. ثلاث خطوات بسيطة.', so: 'WhatsApp waa habka ugu fudud ee loo diro ogeysiisyada gaadiidka. Saddex tallaabo oo fudud.', fr: 'WhatsApp est le moyen le plus simple d\'envoyer des notifications. Trois étapes simples.', zh: 'WhatsApp 是发送配送通知最简单的方式。三个简单步骤即可运行。', uk: 'WhatsApp — найпростіший спосіб надсилати сповіщення. Три прості кроки.' },
  'dashboard.setup.waStep1Label': { en: 'Get a SIM', es: 'Obtener SIM', ar: 'احصل على شريحة', so: 'Hel SIM', fr: 'Obtenir une SIM', zh: '获取 SIM 卡', uk: 'Отримати SIM' },
  'dashboard.setup.waStep2Label': { en: 'Install WhatsApp', es: 'Instalar WhatsApp', ar: 'تثبيت واتساب', so: 'Ku rakib WhatsApp', fr: 'Installer WhatsApp', zh: '安装 WhatsApp', uk: 'Встановити WhatsApp' },
  'dashboard.setup.waStep3Label': { en: 'Link to SafeCare', es: 'Vincular a SafeCare', ar: 'الربط بـ SafeCare', so: 'Ku xidh SafeCare', fr: 'Lier à SafeCare', zh: '链接到 SafeCare', uk: 'Підключити до SafeCare' },
  'dashboard.setup.waGetSimTitle': { en: 'Get a prepaid SIM card', es: 'Obtenga una tarjeta SIM prepaga', ar: 'احصل على شريحة SIM مدفوعة مسبقاً', so: 'Hel kaarka SIM-ka ee lacagta hore', fr: 'Obtenez une carte SIM prépayée', zh: '获取一张预付费 SIM 卡', uk: 'Отримайте передплачену SIM-карту' },
  'dashboard.setup.waGetSimStep1': { en: 'Buy a prepaid SIM card from any store ($3-10). Walmart, Target, gas stations, or dollar stores all carry them.', es: 'Compre una tarjeta SIM prepaga en cualquier tienda ($3-10).', ar: 'اشترِ شريحة SIM مدفوعة مسبقاً من أي متجر (3-10$).', so: 'Ka iibso kaarka SIM-ka dukaan kasta ($3-10).', fr: 'Achetez une carte SIM prépayée dans n\'importe quel magasin (3-10$).', zh: '在任何商店购买一张预付费 SIM 卡（$3-10）。', uk: 'Купіть передплачену SIM-карту в будь-якому магазині ($3-10).' },
  'dashboard.setup.waGetSimStep2': { en: 'Activate the SIM card following the instructions on the packaging. You just need the phone number — no plan needed.', es: 'Active la SIM siguiendo las instrucciones del empaque. Solo necesita el número.', ar: 'قم بتفعيل شريحة SIM حسب التعليمات. تحتاج فقط إلى رقم الهاتف.', so: 'Hawlgeli SIM-ka sida tilmaamaha ku yaal. Kaliya lambarkaad u baahan tahay.', fr: 'Activez la SIM selon les instructions. Vous n\'avez besoin que du numéro.', zh: '按照包装上的说明激活 SIM 卡。您只需要电话号码。', uk: 'Активуйте SIM-карту за інструкцією. Вам потрібен лише номер телефону.' },
  'dashboard.setup.waGetSimStep3': { en: 'Put the SIM in any spare phone — even an old phone works fine. It just needs Wi-Fi and WhatsApp.', es: 'Coloque la SIM en cualquier teléfono disponible. Solo necesita Wi-Fi y WhatsApp.', ar: 'ضع الشريحة في أي هاتف متاح. يحتاج فقط إلى Wi-Fi وواتساب.', so: 'Geli SIM-ka telefoon kasta oo aad hayso. Kaliya Wi-Fi iyo WhatsApp ayuu u baahan yahay.', fr: 'Insérez la SIM dans n\'importe quel téléphone. Il suffit d\'avoir le Wi-Fi et WhatsApp.', zh: '将 SIM 卡放入任何闲置手机中。只需要 Wi-Fi 和 WhatsApp。', uk: 'Вставте SIM-карту в будь-який запасний телефон. Потрібен лише Wi-Fi та WhatsApp.' },
  'dashboard.setup.waSimTip': { en: 'Tip: Use a dedicated prepaid number — not your personal number. This keeps your identity separate and the number can be replaced if needed.', es: 'Consejo: Use un número prepago dedicado, no su número personal.', ar: 'نصيحة: استخدم رقماً مدفوعاً مسبقاً مخصصاً وليس رقمك الشخصي.', so: 'Talo: Isticmaal lambar gaar ah oo lacagta hore ah, maaha lambarkaaga shaqsiga.', fr: 'Conseil : Utilisez un numéro prépayé dédié, pas votre numéro personnel.', zh: '提示：使用专用的预付费号码，而不是您的个人号码。', uk: 'Порада: Використовуйте окремий передплачений номер, а не особистий.' },
  'dashboard.setup.waSimReady': { en: 'I have a SIM card ready', es: 'Tengo una SIM lista', ar: 'لدي شريحة SIM جاهزة', so: 'Waxaan hayaa SIM diyaar ah', fr: 'J\'ai une carte SIM prête', zh: '我已准备好 SIM 卡', uk: 'У мене є готова SIM-карта' },
  'dashboard.setup.waInstallTitle': { en: 'Set up WhatsApp on the phone', es: 'Configure WhatsApp en el teléfono', ar: 'قم بإعداد واتساب على الهاتف', so: 'Ku dejji WhatsApp telefonka', fr: 'Configurez WhatsApp sur le téléphone', zh: '在手机上设置 WhatsApp', uk: 'Налаштуйте WhatsApp на телефоні' },
  'dashboard.setup.waInstallStep1': { en: 'Download WhatsApp from the App Store or Google Play on the phone with the SIM card.', es: 'Descargue WhatsApp de la App Store o Google Play.', ar: 'قم بتنزيل واتساب من متجر التطبيقات.', so: 'Ka soo degso WhatsApp App Store ama Google Play.', fr: 'Téléchargez WhatsApp depuis l\'App Store ou Google Play.', zh: '从 App Store 或 Google Play 下载 WhatsApp。', uk: 'Завантажте WhatsApp з App Store або Google Play.' },
  'dashboard.setup.waInstallStep2': { en: 'Open WhatsApp and verify the new phone number with the SMS code it sends.', es: 'Abra WhatsApp y verifique el número con el código SMS.', ar: 'افتح واتساب وتحقق من الرقم بكود الرسالة.', so: 'Fur WhatsApp oo ku xaqiiji lambarka koodka SMS-ka.', fr: 'Ouvrez WhatsApp et vérifiez le numéro avec le code SMS.', zh: '打开 WhatsApp 并使用短信验证码验证新号码。', uk: 'Відкрийте WhatsApp та підтвердьте номер SMS-кодом.' },
  'dashboard.setup.waInstallStep3': { en: 'Skip the profile setup — you can leave the name blank or put your organization name.', es: 'Omita la configuración del perfil o ponga el nombre de la organización.', ar: 'تجاوز إعداد الملف الشخصي أو ضع اسم المؤسسة.', so: 'Ka bood dejinta profile-ka ama geli magaca ururka.', fr: 'Passez la configuration du profil ou mettez le nom de l\'organisation.', zh: '跳过个人资料设置，或填写组织名称。', uk: 'Пропустіть налаштування профілю або вкажіть назву організації.' },
  'dashboard.setup.waInstallDone': { en: 'WhatsApp is installed and verified', es: 'WhatsApp está instalado y verificado', ar: 'تم تثبيت واتساب والتحقق منه', so: 'WhatsApp waa la rakibay oo la xaqiijiyay', fr: 'WhatsApp est installé et vérifié', zh: 'WhatsApp 已安装并验证', uk: 'WhatsApp встановлено та підтверджено' },
  'dashboard.setup.waLinkTitle': { en: 'Link WhatsApp to SafeCare', es: 'Vincular WhatsApp a SafeCare', ar: 'ربط واتساب بـ SafeCare', so: 'Ku xidh WhatsApp SafeCare', fr: 'Lier WhatsApp à SafeCare', zh: '将 WhatsApp 链接到 SafeCare', uk: 'Підключити WhatsApp до SafeCare' },
  'dashboard.setup.waLinkDesc': { en: 'This links your WhatsApp number to SafeCare so it can send delivery notifications on your behalf.', es: 'Esto vincula su número de WhatsApp a SafeCare para enviar notificaciones.', ar: 'هذا يربط رقم واتساب بـ SafeCare لإرسال الإشعارات.', so: 'Tani waxay ku xidhtaa lambarkaaga WhatsApp SafeCare si ay u dirto ogeysiisyada.', fr: 'Ceci lie votre numéro WhatsApp à SafeCare pour envoyer des notifications.', zh: '这将您的 WhatsApp 号码链接到 SafeCare 以发送通知。', uk: 'Це підключає ваш номер WhatsApp до SafeCare для надсилання сповіщень.' },
  'dashboard.setup.waLinkInstructions': { en: 'On the phone with WhatsApp:', es: 'En el teléfono con WhatsApp:', ar: 'على الهاتف الذي عليه واتساب:', so: 'Telefonka WhatsApp ku jira:', fr: 'Sur le téléphone avec WhatsApp :', zh: '在安装了 WhatsApp 的手机上：', uk: 'Н�� телефоні з WhatsApp:' },
  'dashboard.setup.waLinkStep1': { en: 'Open WhatsApp Settings (three dots menu > Linked Devices)', es: 'Abra Configuración de WhatsApp (menú > Dispositivos vinculados)', ar: 'افتح إعدادات واتساب (القائمة > الأجهزة المرتبطة)', so: 'Fur Dejinta WhatsApp (menu > Aaladaha la xidhiidhiyay)', fr: 'Ouvrez les paramètres WhatsApp (menu > Appareils liés)', zh: '打开 WhatsApp 设置（菜单 > 关联设备）', uk: 'Відкрийте Налаштування WhatsApp (меню > Пов\'язані пристрої)' },
  'dashboard.setup.waLinkStep2': { en: 'Tap "Link a Device"', es: 'Toque "Vincular un dispositivo"', ar: 'انقر على "ربط جهاز"', so: 'Taabo "Ku xidh aalad"', fr: 'Appuyez sur « Lier un appareil »', zh: '点击"关联设备"', uk: 'Натисніть "Під\'єднати пристрій"' },
  'dashboard.setup.waLinkStep3': { en: 'Point the phone camera at the QR code below', es: 'Apunte la cámara al código QR de abajo', ar: 'وجّه كاميرا الهاتف إلى رمز QR أدناه', so: 'U jeedi kamaradda telefonka koodka QR ee hoose', fr: 'Dirigez la caméra vers le code QR ci-dessous', zh: '将手机摄像头对准下方的二维码', uk: 'Направте камеру телефону на QR-код нижче' },
  'dashboard.setup.waQrScanPrompt': { en: 'Scan this code with WhatsApp on the phone', es: 'Escanee este código con WhatsApp', ar: 'امسح هذا الرمز بواتساب', so: 'Sawir koodkan WhatsApp-ka telefonka', fr: 'Scannez ce code avec WhatsApp', zh: '使用手机上的 WhatsApp 扫描此代码', uk: 'Скануйте цей код за допомогою WhatsApp' },
  'dashboard.setup.waWaitingForQr': { en: 'Connecting to WhatsApp... QR code will appear shortly.', es: 'Conectando a WhatsApp... El código QR aparecerá pronto.', ar: 'جارٍ الاتصال بواتساب... سيظهر رمز QR قريباً.', so: 'Ku xidhnaya WhatsApp... Koodka QR wuu soo muuqan doonaa.', fr: 'Connexion à WhatsApp... Le code QR apparaîtra bientôt.', zh: '正在连接 WhatsApp... 二维码即将出现。', uk: 'Підключення до WhatsApp... QR-код з\'явиться незабаром.' },
  'dashboard.setup.waConnected': { en: 'WhatsApp connected successfully!', es: '¡WhatsApp conectado exitosamente!', ar: 'تم الاتصال بواتساب بنجاح!', so: 'WhatsApp si guul leh ayaa loo xidhay!', fr: 'WhatsApp connecté avec succès !', zh: 'WhatsApp 连接成功！', uk: 'WhatsApp підключено успішно!' },
  'dashboard.setup.waStartPairing': { en: 'Show QR Code to Pair', es: 'Mostrar código QR para emparejar', ar: 'عرض رمز QR للاقتران', so: 'Tus koodka QR si aad u lamaane noqoto', fr: 'Afficher le code QR pour jumeler', zh: '显示配对二维码', uk: 'Показати QR-код для з\'єднання' },
  'dashboard.setup.advancedChannels': { en: 'Advanced: Signal & SMS channels', es: 'Avanzado: canales Signal y SMS', ar: 'متقدم: قنوات Signal و SMS', so: 'Horumarsan: kanaalada Signal & SMS', fr: 'Avancé : canaux Signal et SMS', zh: '高级：Signal 和 SMS 频道', uk: 'Додатково: канали Signal та SMS' },

  // --- WhatsApp pool management (settings page) ---
  'dashboard.settings.waPoolTitle': { en: 'WhatsApp Lines', es: 'Líneas de WhatsApp', ar: 'خطوط واتساب', so: 'Khadadka WhatsApp', fr: 'Lignes WhatsApp', zh: 'WhatsApp 线路', uk: 'Лінії WhatsApp' },
  'dashboard.settings.waPoolDesc': { en: 'Manage WhatsApp numbers for notifications and blind driver-recipient communication.', es: 'Gestione números de WhatsApp para notificaciones y comunicación ciega.', ar: 'إدارة أرقام واتساب للإشع��رات والتواصل المجهول.', so: 'Maamul lambarada WhatsApp ee ogeysiisyada iyo xiriirka indho-tirka.', fr: 'Gérez les numéros WhatsApp pour les notifications et la communication anonyme.', zh: '管理 WhatsApp 号码，用于通知和匿名通信。', uk: 'Керуйте номерами WhatsApp для сповіщень та анонімного зв\'язку.' },
  'dashboard.settings.waConnectedCount': { en: 'connected', es: 'conectados', ar: 'متصل', so: 'ku xidhan', fr: 'connecté(s)', zh: '已连接', uk: 'підключено' },
  'dashboard.settings.waAddLine': { en: 'Add Line', es: 'Agregar línea', ar: 'إضافة خط', so: 'Ku dar khadka', fr: 'Ajouter une ligne', zh: '添加线路', uk: 'Додати лінію' },
  'dashboard.settings.waNewLine': { en: 'Add a new WhatsApp line', es: 'Agregar nueva línea', ar: 'إضافة خط جديد', so: 'Ku dar khadka cusub', fr: 'Ajouter une nouvelle ligne', zh: '添加新线路', uk: 'Додати нову лінію' },
  'dashboard.settings.waLineLabelPlaceholder': { en: 'e.g., Main Line, Relay #1', es: 'ej., Línea principal, Relay #1', ar: 'مثال: خط رئيسي، ريلاي #1', so: 'tusaale, Khadka weyn, Relay #1', fr: 'ex., Ligne principale, Relais #1', zh: '例如：主线路、中继 #1', uk: 'напр., Основна лінія, Реле #1' },
  'dashboard.settings.waRelayPoolCheckbox': { en: 'Use as relay line (blind driver-recipient communication)', es: 'Usar como línea de retransmisión (comunicación ciega)', ar: 'استخدام كخط ترحيل (اتصال مجهول)', so: 'U isticmaal sidii khadka wareejinta (xiriir indho-tir ah)', fr: 'Utiliser comme ligne relais (communication anonyme)', zh: '用作中继线路（匿名通信）', uk: 'Використовувати як реле (анонімний зв\'язок)' },
  'dashboard.settings.waNoLines': { en: 'No WhatsApp lines configured yet.', es: 'Aún no hay líneas de WhatsApp configuradas.', ar: 'لم يتم تكوين خطوط واتساب بعد.', so: 'Wali lama dejinin khadad WhatsApp.', fr: 'Aucune ligne WhatsApp configurée.', zh: '尚未配置 WhatsApp 线路。', uk: 'Ще не налаштовано жодної лінії WhatsApp.' },
  'dashboard.settings.waAddFirstLine': { en: 'Add Your First WhatsApp Line', es: 'Agregue su primera línea', ar: 'أضف أول خط واتساب', so: 'Ku dar khadkaaga koowaad', fr: 'Ajoutez votre première ligne', zh: '添加您的第一条线路', uk: 'Додайте першу лінію WhatsApp' },
  'dashboard.settings.waPrimary': { en: 'Primary', es: 'Principal', ar: 'رئيسي', so: 'Aasaasi', fr: 'Principal', zh: '主要', uk: 'Основна' },
  'dashboard.settings.waRelay': { en: 'Relay', es: 'Relay', ar: 'ريلاي', so: 'Wareejin', fr: 'Relais', zh: '中继', uk: 'Реле' },
  'dashboard.settings.waQrScan': { en: 'Scan with WhatsApp on the phone to link', es: 'Escanee con WhatsApp para vincular', ar: 'امسح بواتساب على الهاتف للربط', so: 'Ku sawir WhatsApp-ka telefonka si aad u xidhidhiso', fr: 'Scannez avec WhatsApp pour lier', zh: '使用手机 WhatsApp 扫描以链接', uk: 'Скануйте WhatsApp на телефоні для підключення' },
  'dashboard.settings.waConnect': { en: 'Connect', es: 'Conectar', ar: 'اتصال', so: 'Ku xidh', fr: 'Connecter', zh: '连接', uk: 'Підключити' },
  'dashboard.settings.waConnecting': { en: 'Connecting...', es: 'Conectando...', ar: 'جارٍ الاتصال...', so: 'Ku xidhaya...', fr: 'Connexion...', zh: '连接中...', uk: 'Підключення...' },
  'dashboard.settings.waDisconnect': { en: 'Disconnect', es: 'Desconectar', ar: 'قطع الاتصال', so: 'Ka jar', fr: 'Déconnecter', zh: '断开', uk: 'Відключити' },
  'dashboard.settings.waSetPrimary': { en: 'Set as Primary', es: 'Establecer como principal', ar: 'تعيين كرئيسي', so: 'U dhig aasaasi', fr: 'Définir comme principal', zh: '设为主要', uk: 'Зробити основною' },
  'dashboard.settings.waAddToPool': { en: 'Add to Relay Pool', es: 'Agregar al pool de relay', ar: 'إضافة إلى مجموعة الترحيل', so: 'Ku dar pool-ka wareejinta', fr: 'Ajouter au pool relais', zh: '添加到中继池', uk: 'Додати до пулу реле' },
  'dashboard.settings.waRemoveFromPool': { en: 'Remove from Pool', es: 'Quitar del pool', ar: 'إزالة من المجموعة', so: 'Ka saar pool-ka', fr: 'Retirer du pool', zh: '从池中移除', uk: 'Видалити з пулу' },
  'dashboard.settings.waRemoveLine': { en: 'Remove', es: 'Eliminar', ar: 'إزالة', so: 'Ka saar', fr: 'Supprimer', zh: '删除', uk: 'Видалити' },
  'dashboard.settings.waRelayPoolStatus': { en: 'Relay Pool', es: 'Pool de relay', ar: 'مجموعة الترحيل', so: 'Pool-ka wareejinta', fr: 'Pool relais', zh: '中继池', uk: 'Пул реле' },
  'dashboard.settings.waRelayLinesReady': { en: 'relay lines connected and ready', es: 'líneas relay conectadas y listas', ar: 'خطوط ترحيل متصلة وجاهزة', so: 'khadad wareejin oo diyaar ah', fr: 'lignes relais connectées et prêtes', zh: '中继线路已连接就绪', uk: 'ліній реле підключено та готово' },
  'dashboard.settings.waRelayExplanation': { en: 'Relay lines let drivers and recipients message each other through an anonymous WhatsApp number — neither sees the other\'s real number.', es: 'Las líneas relay permiten que conductores y destinatarios se comuniquen a través de un número anónimo.', ar: 'خطوط الترحيل تتيح للسائقين والمستلمين التواصل عبر رقم مجهول.', so: 'Khadadka wareejinta waxay u oggolaadaan darawallada iyo qaabilayaasha inay isfariin iyagoo isticmaalaya lambar qarsoon.', fr: 'Les lignes relais permettent aux chauffeurs et destinataires de communiquer via un numéro anonyme.', zh: '中继线路让司机和收件人通过匿名 WhatsApp 号码互相发消息。', uk: 'Лінії реле дозволяють водіям та отримувачам спілкуватися через анонімний номер WhatsApp.' },
};
